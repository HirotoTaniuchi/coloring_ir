...
def train_model(net, dataloaders_dict, criterion, scheduler, optimizer, num_epochs, n_layers=50, num_classes=21):
    """
    ...
    for epoch in range(num_epochs):
        ...
        epoch_val_miou = 0.0  # epochのmIoU和
            ...
            for imges, anno_class_imges in dataloaders_dict[phase]:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    # 検証時
                        ...
                        miou = calculate_miou(outputs_out, anno_class_imges, num_classes)
                        epoch_val_miou += miou

        # epochのphaseごとのlossと正解率
        t_epoch_finish = time.time()
        print('-------------')
        print('epoch {} || Epoch_TRAIN_Loss:{:.4f} ||Epoch_VAL_Loss:{:.4f} ||Epoch_VAL_mIoU:{:.4f} ||{}'.format(
            epoch+1, epoch_train_loss/num_train_imgs, epoch_val_loss/num_val_imgs, epoch_val_miou/num_val_imgs, now1()))
        print('timer:  {:.4f} sec.'.format(t_epoch_finish - t_epoch_start))
        t_epoch_start = time.time()

        # ログを保存
        log_epoch = {'epoch': epoch+1, 'train_loss': epoch_train_loss /
                     num_train_imgs, 'val_loss': epoch_val_loss/num_val_imgs, 'val_miou': epoch_val_miou/num_val_imgs}
        
        logs.append(log_epoch)
        writer.add_scalar('train_loss', logs[epoch]['train_loss'], epoch+1)
        writer.add_scalar('val_loss', logs[epoch]['val_loss'], epoch+1)
        writer.add_scalar('val_miou', logs[epoch]['val_miou'], epoch+1)
