...
class Unet_Discriminator(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        return torch.sigmoid(out), torch.sigmoid(bottleneck_out)


def D_unet_arch2(ch=64, attention='64',ksize='333333', dilation='111111',out_channel_multiplier=1):
    arch = {}
    ...
    arch[256] = {'in_channels' :            [3] + [ch*item for item in [1, 2, 4, 8, 8, 8*2, 4*2, 2*2, 1*2, 1]],
                             'out_channels' : [item * ch for item in [1, 2, 4, 8, 8, 8, 4, 2, 1, 1]],
                             'downsample' : [True] *5 + [False]*5 ,
                             'upsample':    [False]*5 + [True] *5,
                             'resolution' : [128, 64, 32, 16, 8, 16, 32, 64, 128, 256 ],
                             'attention' : {2**i: 2**i in [int(item) for item in attention.split('_')]
                                                            for i in range(2,13)}}
...
class Unet_Discriminator2(nn.Module):
    ...
    def __init__(self, D_ch=64, D_wide=True, resolution=256,
                             ...
                             SN_eps=1e-12, output_dim=1, D_mixed_precision=False, D_fp16=False,
                             D_init='ortho', skip_init=False, D_param='SN', decoder_skip_connection = True, **kwargs):
        super(Unet_Discriminator2, self).__init__()
        ...
        self.out_channel_multiplier = 1#4
        # Architecture
        self.arch = D_unet_arch2(self.ch, self.attention , out_channel_multiplier = self.out_channel_multiplier  )[resolution]
