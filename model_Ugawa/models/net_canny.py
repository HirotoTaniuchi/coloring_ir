...
if __name__ == '__main__':
    ...
    #input = torch.randn((batch_size, input_nc, H, W)).to('cuda:0')
    input = Image.open('set03_V000_I00135.jpg').convert('RGB')
    print(type(input))
    input_t = to_tensor(input)[None, ...].to('cuda:0')
    sharpening = ImageSharpening()    
    
    normalized = transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))(input_t)
    
    fake = sharpening(input_t)
    fake = transforms.Normalize((0.5,), (0.5,))(torch.clip(fake, 0, 1))
    sharpened = unsharp_mask(input, radius=1, amount=8)
    #sharpened = transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))(sharpened)
    img = to_pil_image(fake[0])
    img.save('sharpened.jpg')

    img = to_pil_image(normalized[0])
    img.save('normalized.jpg')
    input_t = torch.from_numpy(np.transpose(sharpened, (2,0,1)))
    input_t = transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))(input_t)
    sharp = to_pil_image(input_t)
    sharp.save('aaa_2.jpg')
