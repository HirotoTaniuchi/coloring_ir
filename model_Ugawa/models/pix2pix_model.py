...
import random
import kornia as K
from BASNet.model.BASNet import BASNet # saliency顕著生マップの計算で利用
# もし使いたい場合はprojects/coloring_ir/20250227_Ugawa/BASNetのコードをprojects/coloring_ir/20251105_Ugawa/BASNetにコピーする
import torchvision
...
class Pix2PixModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if ('AAFSTNet2' in opt.which_model_netG) and (opt.phase == 'train') and not('NoSeg' in opt.which_model_netG):
        ...
        if self.isTrain:
            ...
            # stage==fullのときはD_instのoptimizerを追加
            if opt.stage == 'full' and (self.opt.w_gan_inst > 0 or self.opt.w_gan_seg > 0):
    ...
    def load_segmentation_model(self):
        print("let's load segmentation model")
    ...
    def get_segmentation_feature(self):
        with torch.no_grad():
            ...
            self.edge = decoded_pred.float().detach()
    ...
    def forward(self, bboxes=0):
        ...
        elif 'AAFSTNet2andSegNet2_RGB' == self.opt.which_model_netG:
            self.fake_B = self.netG(self.real_A, self.edge, input_size=(self.opt.fineSize_H, self.opt.fineSize_W), seg_pred=None)
        elif 'AAFSTNet2andSegNet2_NoSeg' == self.opt.which_model_netG:
            self.fake_B = self.netG(self.real_A, None, input_size=(self.opt.fineSize_H, self.opt.fineSize_W), seg_pred=None)
        elif 'AAFSTNet2andSegNet2' in self.opt.which_model_netG:
    ...
    def backward_D_inst(self):
        ...
        # GPがある場合はloss_DにGradient Penaltyを追加
        if 'GP' in self.opt.gan_type:
            self.loss_D_inst += networks.cal_gradient_penalty(self.netD_inst, real_AB_inst, fake_AB_inst, self.device, type='mixed', constant=1.0, lambda_gp=self.opt.lambda_gp)[0]
        ...
        if 'GP' in self.opt.gan_type:
            ...
            retain_graph = False
        self.loss_D_inst.backward(retain_graph=retain_graph)
    ...
    def optimize_parameters(self, bbox=None, skip=False):
        #with torch.autograd.detect_anomaly():
            ...
            # D_instの更新
            for i in range(n_critic):
                if self.opt.stage == 'full' and ((self.opt.w_gan_inst > 0 and bbox != 0) or self.opt.w_gan_seg > 0):
                    if (not self.skip) and self.use_inst:
                        if self.use_gan ==1:
