...
class Training_Full_Dataset(Data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        output = {}
        print(f"output[rgb_img].shape = {self.transforms(rgb_img).shape}")
        print(f"output[gray_img].shape = {self.transforms(gray_img).shape}")
...
class Training_Full_Dataset_TIR(Data.Dataset):
    '''
    Training on COCOStuff dataset. [train2017.zip]
    ...
    def __getitem__(self, index):
        ...
        # aligned_dataset.pyより
        AB_path = join(self.IMAGE_DIR, self.IMAGE_ID_LIST[index])
