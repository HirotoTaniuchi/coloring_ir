...
##############################################################
DATE=2024_1_8
MODEL="AAFSTNet2andSegNet2"    # model name (str)
NET_D="basic"
OPTION="_RSGAN_LabelToFAB"    # additional information like 「wl_0.5」    _seg1_conv_segFilter10
...
#DATASET_DIR="/home/usrs/ugawa/lab/work/datasets/IRVI/single/rooftop_total/AB/train"
#DATASET_DIR_A="/home/usrs/ugawa/lab/work/datasets/IRVI/single/rooftop_total/trainA"
#DATASET_DIR_B="/home/usrs/ugawa/lab/work/datasets/IRVI/single/rooftop_total/trainB"
#SEG_CKPT_PATH=/home/usrs/ugawa/lab/work/DeepLabV3Plus-Pytorch/checkpoints/KAIST_finetune_40000/latest_deeplabv3plus_resnet101_cityscapes_os16.pth
#SEG_CKPT_PATH=/home/usrs/ugawa/lab/work/DeepLabV3Plus-Pytorch/checkpoints_scratch_Thermal/FLIR/best_deeplabv3plus_resnet101_cityscapes_os16.pth
...
SAL_MAP="None" # saliency or sod None
...
SEG_TO_D="--seg_to_D"  # --seg_to_D
SEG_TO_D_INST="--seg_to_D_inst"  # --seg_to_D_inst
SEG_TO_FAB="--seg_to_FAB"  # --seg_to_FAB

USE_SEGMAP="--use_segmap"    # --use_segmap
USE_SEGMAP_PROB=0.75    # --use_segmap_prob (float)
...
GPU_ID=1
cp train_seg2.sh ${LOCALTMP}/checkpoints/${DATE}_${MODEL}_${DATASET_NAME}/${MODEL}_${DATASET_NAME}_${STAGE}${OPTION}/train_seg2.sh
wait


python train.py --dataroot $DATASET_DIR --dataroot_A $DATASET_DIR_A --dataroot_B $DATASET_DIR_B --seg_dir $SEG_DIR --seg_ckpt_path $SEG_CKPT_PATH --dataset_mode unaligned --name ${MODEL}_${DATASET_NAME}_${STAGE}${OPTION} --gpu_ids $GPU_ID --which_model_netG ${MODEL} --which_model_netD $NET_D --nThreads $NTHREADS --input_nc $INPUT_NC --output_nc $OUTPUT_NC --checkpoints_dir ${LOCALTMP}/checkpoints/${DATE}_${MODEL}_${DATASET_NAME} $EPOCH_COUNT $CONTINUE_TRAIN $WHICH_EPOCH $NEED_BBOX $D_LABEL $D_INST_LABEL --phase train --loadSize_W $LOADSIZE --loadSize_H $LOADSIZE --fineSize_W $FINESIZE --fineSize_H $FINESIZE --transform $TRANSFORM --l1 $L1 --w_l1 $W_L1 --w_gan $W_GAN --w_tv $W_TV --w_vgg $W_VGG --w_edge $W_EDGE --w_cx $W_CX --w_con $W_CON --w_l1_inst $W_L1_INST --w_gan_inst $W_GAN_INST --w_vgg_inst $W_VGG_INST --w_cx_inst $W_CX_INST --w_reg $W_reg --w_sal $W_SAL --w_cd $W_CD --w_seg $W_SEG --w_ssim $W_SSIM --w_gan_seg $W_GAN_SEG --gan_type $GAN_TYPE --niter $HALF_EPOCH --niter_decay $HALF_EPOCH --bbox_threshold $BBOX --stage $STAGE  $TENSORBOARD $WANDB --display_env $VISDOM_ENV --n_layers_D $N_LAYERS_D --display_port $VISDOM_PORT $MIRROR $A_INVERT $A_INVERT_PROB $OUT_GRAY --sal_map $SAL_MAP $NORMALIZE_SAL $INPUT_SOD $SAL_MASK $USE_CONDITION --colorspace $COLORSPACE --l1_channel $L1_CHANNEL $L1_BGONLY --display_freq $DISPLAY_FREQ $USE_SEGMAP --use_segmap_prob $USE_SEGMAP_PROB --seg_mask_size $SEG_MASK_SIZE --seg_mask_ratio $SEG_MASK_RATIO $SEG_TO_D $SEG_TO_D_INST --seg_num_classes $SEG_NUM_CLASSES $SEG_FILTER --seg_filter_radius $SEG_FILTER_RADIUS --seg_ckpt_path $SEG_CKPT_PATH $USE_SEG_MODEL --input_type_for_seg_model $INPUT_TYPE_FOR_SEG_MODEL $SEG_TO_FAB $CLAHE
