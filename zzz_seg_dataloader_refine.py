...
from torchvision.transforms import ToTensor, Compose, RandomRotation, RandomHorizontalFlip, Resize, Normalize



class DataTransformRGB():
    """
...
class DataTransformSeg():
    ...
    def __call__(self, phase, anno_class_img):
        """
        Parameters
        ----------
        phase : 'train' or 'val'
            前処理のモードを指定。
        """
        return self.data_transform[phase](anno_class_img)
...
class MFNetDataset(data.Dataset):
    ...
    def __init__(self, img_list, anno_list, phase, transform_rgb, transform_seg, color_mean, color_std):
    ...
    def pull_item(self, index):
        ...
        # 3. 前処理を実施
        img = self.transform_rgb(self.phase, img, self.color_mean, self.color_std)
        anno_class_img = self.transform_seg(self.phase, anno_class_img)
...
if __name__ == "__main__":
    ...
    # 動作の確認
    for imges, anno_class_imges in train_dataloader:
        print("a")
